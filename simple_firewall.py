...
# tested with Pica8-3822 and NoviFlow-2128 switches
#--------------------------------------------------------------------
...
# to store ARP table: dpid->dst_MAC->port_number
mac_to_port = {}
...
class FirewallController(ControllerBase):
    ...
    def packet_in_handler(msg):
        FirewallController._LOGGER.info('-------------> in packet_in')


        #learn what is connected to where ----------------
        ...
        FirewallController._LOGGER.info("dst: %s src: %s|MAC table: %s"%(dst, src, mac_to_port))    
        ...
        if eth.ethertype == ether.ETH_TYPE_ARP:
            ...
            if out_port != ofproto.OFPP_FLOOD:
                match = parser.OFPMatch(in_port=in_port, eth_type=ether.ETH_TYPE_ARP, eth_dst=dst)
                # verify if we have a valid buffer_id, if yes avoid to send both
                # flow_mod & packet_out
                if msg.buffer_id != ofproto.OFP_NO_BUFFER:
                    FirewallController.add_flow(datapath, 1, match, actions, msg.buffer_id)
                    return
                else:
                    FirewallController.add_flow(datapath, 1, match, actions)
            data = None
            if msg.buffer_id == ofproto.OFP_NO_BUFFER:
                data = msg.data
        ...
        #### ---
            ...
            FirewallController._LOGGER.info('-------------> FIREWALL WORKS: dpid=%s: Blocked packet = %s', dpid_str, pkt)
...
class Firewall(object):
    ...
    def _set_rule(self, cookie, rest, waiters, vlan_id):
        ...
        if 'port'in actions[0] and actions[0]['port']=='NORMAL':
            # ! port:NORMAL action not supported by NS2128

            dpid = self.dp.id
            if (dpid) in topology_conf:

                if "nw_dst" in match:
                    
                    out_port= int(topology_conf[dpid][match["nw_dst"]])
                    actions = [{'type': 'OUTPUT','port': out_port}]
                    FirewallController._LOGGER.info("Apply new action:")

                else:
                    FirewallController._LOGGER.error("ERROR: This type of match not supported")
                    return

            else:
                FirewallController._LOGGER.error("ERROR: no DPID in the topology_conf!")
                return
...
class Action(object):
    ...
    def to_rest(openflow):
        if REST_ACTION in openflow:
            ...
            if openflow[REST_ACTION] != ['OUTPUT:CONTROLLER']:
